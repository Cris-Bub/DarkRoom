...
- V1 `ToneTuning` and Tone Lab V2 `BehaviorTuning` defaults plus tuning-to-kernel mapping for developer experiments, including per-slider response mappings, signed endpoint/range width, influence curve body/peak shaping, camera-like exposure feel, exposure-shadow range shaping, regional chroma modes, centered color-coupling/protection controls, and overlay parameters.
- C ABI exports that the macOS target can link against for tonal parameter math, Tone Lab response diagnostics, histogram binning, and per-pixel tone recipe application used by the interactive histogram fast path.
...
- Keep visible slider to renderer-parameter mapping and hidden behavior-tuning derivation in Rust so Swift UI can tune constants without owning response-curve, exposure-shadow range, regional chroma-mode, centered color-protection, gamut-compression, or color-coupling math.
